...
const common = {
  getJob: async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      res.status(500).send({ error: true, message: `Error getting the data from database ${e}` })
    }
  },
  ...
  allEmployee: async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      logger.error({ error: e, message: e.message })
      res.status(500).json({ error: true, message: `Error getting the data from database ${e}` })
    }
  },
}

...
const AddFacility = ({navigation}) => {
  ...
  function handleAddFacility() {
    ...
    if (img.length != 0) {
      img.map(item => formData.append(`Image`, item));
    }
    ...
    CFormFetch('/Management/CreateFacility', token, formData)
      .then(res => {
  ...
  return (
    <>
      <WaitModal modalVisible={!items.isLoaded} />
      {!isInternet || isError ? (
        <NoInternet
          refresh={refresh}
      ...
      {items.isClicked == false && items.isInternet == true ? (
        <View
          style={{justifyContent: 'flex-end', backgroundColor: Colors.blue}}>
          <Button
            title="Save"
